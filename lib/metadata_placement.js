 ...
 *
 * Some notes on the optional `opts.mdapiClientsInAdvance` argument:
 * - The default value is `true`.
 * - If this is `true`, the MetadataPlacement object will open client
 *   connections to each mdapi instance in advance.
 * - If this is false, mdapi clients will be opened as-needed, and will then
 *   persist until the close() method is called.
 * - This option should be set according to the anticipated use pattern of the
 *   MetadataPlacement object. For a long-lasting MetadataPlacement object that
 *   will be handling many requests to many mdapi instances, where it is
 *   favorable to avoid per-request latency, set it to `true`.
 *   For a short-lived MetadataPlacement object that will only be handling a few
 *   requests, set it to `false`.
 */
function MetadataPlacement(opts) {
    ...
    assert.optionalBool(opts.mdapiClientsInAdvance,
        'options.mdapiClientsInAdvance');
    ...
    this.mdapiClientsInAdvance = opts.mdapiClientsInAdvance !== false;
    this.mdapiClientsInAdvance = false;
    ...
    this.buckets_mdplacement_client.once('connect', function _onConnect() {
        ...
        self.buckets_mdplacement_client.getPlacementData(function (pErr, pd) {
            ...
            if (self.mdapiClientsInAdvance) {
                ...
                var clients = buckets_mdapi_client.createClient({
                    pnodes: pnodes,
                    bucketsMdapiOptions:
                        self.cfg.buckets_mdapi.bucketsMdapiOptions,
                    log: self.log,
 ...
 */
 // TODO maybe factor out duplicate client-creation code?
MetadataPlacement.prototype.getBucketsMdapiClient =
    ...
    if (!this.buckets_mdapi_clients.map[pnode]) {
        assert.equal(this.mdapiClientsInAdvance, false,
            'mdapi client should already exist for pnode %s because ' +
            'this.mdapiClientsInAdvance is false', pnode);
        self.log.info('creating buckets-mdapi client');
        ...
        self.log.info('created buckets-mdapi client for pnode %s', pnode);
        assert.equal(clients.array.length, 1, 'clients.array.length === 1');
        var client = clients.array[0];
        /*
         * Add the client to the map and array of clients. There won't be any
         * duplicates because we only create the client if it didn't exist in
         * the first place.
         */
        self.buckets_mdapi_clients.map[pnode] = client;
        self.buckets_mdapi_clients.array.push(client);
    }
...
MetadataPlacement.prototype.getAllNodes = function getAllNodes() {
    ...
    if (!self.mdapiClientsInAdvance) {
        var all_pnodes = getPnodes(self.placementData);
        var missing_pnodes = all_pnodes.filter(function (pnode) {
            return !self.buckets_mdapi_clients.map[pnode];
        });
        self.log.info('creating missing buckets-mdapi clients');

        var clients = buckets_mdapi_client.createClient({
            pnodes: [missing_pnodes],
            bucketsMdapiOptions: self.cfg.buckets_mdapi.bucketsMdapiOptions,
            log: self.log,
            crc_mode: fast.FAST_CHECKSUM_V2
        });
        self.log.info('created missing buckets-mdapi clients', pnode);
        /*
         * Add the new clients to the existing map and array of clients. There
         * won't be any duplicates because we only create the client if it
         * didn't exist in the first place.
         */
        Object.entries(clients.map).forEach(function (tuple) {
            var pnode = tuple[0];
            var client = tuple[1];
            self.buckets_mdapi_clients.map[pnode] = client;
            self.buckets_mdapi_clients.array.push(client);
